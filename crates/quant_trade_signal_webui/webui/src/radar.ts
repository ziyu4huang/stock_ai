...
export function render(v: TabSnapshot): void {
  ...
  radarEl.innerHTML = `
    ...
    <div class="conf-bar-wrap">
      <div class="conf-label"><span style="color:var(--dim)">${t('confidence')}</span><span style="color:${confColor}">${conf.toFixed(0)}%</span></div>
      <div class="conf-bar"><div class="conf-fill" style="background:${confColor};width:${conf.toFixed(0)}%"></div></div>
    </div>

    <div class="market-state" style="color:${stateColor}">${v.state_icon} ${stateLabel}</div>
    <div class="hmm-state">HMM <span style="color:${hmmColor}">${v.hmm_state_icon} ${hmmLabel}</span> <span class="${v.hmm_agrees ? 'hmm-agree' : 'hmm-div'}">${v.hmm_agrees ? '\u2713 ' + t('agree') : '\u2717 ' + t('div')}</span></div>
    <div class="action-box" style="border-color:${actionColor}30">
      ...
      <div class="sl-label">${v.stop_loss !== null ? t('sl') + v.stop_loss.toFixed(2) : t('sl.dash')}</div>
    </div>

    <!-- Spoof / cluster warnings -->
    ${hasSpoof ? `<div class="spoof-alert">${t('spoof')}${v.suspicious_removes}${t('spoof.detail')}${v.suspicious_adds}${t('surge')}</div>` : ''}
    ${hasCluster ? `<div class="spoof-alert">${t('cluster')}${v.cluster_count}${t('cluster.detail')}</div>` : ''}

    <!-- Whale volume breakdown (volume + count) -->
    <div style="margin-top:4px">
      <div class="counter-row"><span class="counter-label">${t('whale.buy')}</span><div class="counter-bar" style="background:rgba(105,240,174,0.3);width:${(v.whale_buys / maxWhale * 100).toFixed(0)}%"></div><span class="counter-val" style="color:var(--lgreen)">${v.whale_buy_volume_m.toFixed(1)}M (${v.whale_buys})</span></div>
      <div class="counter-row"><span class="counter-label">${t('whale.sell')}</span><div class="counter-bar" style="background:rgba(255,82,82,0.3);width:${(v.whale_sells / maxWhale * 100).toFixed(0)}%"></div><span class="counter-val" style="color:var(--lred)">${v.whale_sell_volume_m.toFixed(1)}M (${v.whale_sells})</span></div>
      <div class="counter-row"><span class="counter-label">${t('ign.long')}</span><div class="counter-bar" style="background:rgba(76,175,80,0.3);width:${(v.long_ignitions / maxWhale * 100).toFixed(0)}%"></div><span class="counter-val" style="color:var(--green)">${v.long_ignitions}</span></div>
      <div class="counter-row"><span class="counter-label">${t('ign.short')}</span><div class="counter-bar" style="background:rgba(239,83,80,0.3);width:${(v.short_ignitions / maxWhale * 100).toFixed(0)}%"></div><span class="counter-val" style="color:var(--red)">${v.short_ignitions}</span></div>
    </div>
    ...
    <div class="price-row">
      ...
      <span style="color:var(--dim)">${t('ticks')}${v.total_ticks}</span>
    </div>

    <!-- Velocity indicator -->
    <div class="velocity-ind ${velClass}" style="color:${velColor}">${t('velocity')}${vel.toFixed(1)}${t('tmin')}${vel > 60 ? '\u26A1' : vel > 30 ? '\u25B2' : ''}</div>

    <!-- Absorption score -->
    ${v.absorption_score > 20 ? `<div style="font-size:10px;color:var(--cyan);padding:1px 0">${t('absorption')}${v.absorption_score.toFixed(0)}%</div>` : ''}
