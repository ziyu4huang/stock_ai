#!/usr/bin/env bash
########################################
# GLM AI Assistant Wrapper Script (Team Edition)
#
# This script provides a wrapper function 'glm-team' that interfaces with the GLM AI model
# through an Anthropic-compatible API endpoint with Claude Team features enabled.
...
#
#   3. Use the glm-team command:
#      glm-team "your prompt here"
#      glm-team --help  # for claude help
#
# FEATURES:
# - Runs in subshell to avoid side effects on your shell environment
# - Validates API key before execution
# - Claude Team features enabled (shared context, team memory, etc.)
# - Uses default models if not overridden
#
# CUSTOMIZATION:
# Override these environment variables if needed:
#   Z_AI_MODEL_DEFAULT="glm-5"      # Default model to use
#   Z_AI_MODEL_ALTERNATIVE="glm-5"  # Alternative model
#   Z_AI_MODE="ZAI"                 # API mode
#   CLAUDE_TEAM_ID="your-team-id"   # Team ID for team features
########################################

# Default values (only set if not already defined)
: "${Z_AI_MODE:="ZAI"}"
: "${Z_AI_MODEL_OPUS:="glm-5"}"
: "${Z_AI_MODEL_DEFAULT:="glm-5"}"
: "${Z_AI_MODEL_AIR:="glm-4.5-air"}"
: "${Z_AI_MODEL_ALTERNATIVE:="glm-5"}"

# Team-specific configuration
: "${CLAUDE_TEAM_ID:="claude-teamate"}"
: "${CLAUDE_TEAM_MEMORY:="true"}"

glm-team()
{
  # Check if Z_AI_API_KEY is defined
  if [ -z "${Z_AI_API_KEY:-}" ]; then
    echo "Error: Z_AI_API_KEY is not defined. Please set it before running glm-team." >&2
  ...
  (
    ...
    export CLAUDE_TEAM="${CLAUDE_TEAM_ID}"
    export CLAUDE_USE_TEAM="${CLAUDE_TEAM_MEMORY}"
    export CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1

    echo "GLM Team mode enabled. Model: ${ANTHROPIC_MODEL}, Team: ${CLAUDE_TEAM}"
